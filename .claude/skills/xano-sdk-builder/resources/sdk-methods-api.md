...
{
  "operations": [
    {"method": "apiRequest", "args": ["https://api.example.com/users", "POST", {"headers": {"Authorization": "Bearer $env.API_KEY", "Content-Type": "application/json"}, "body": {"name": "$user_name", "email": "$user_email"}}, "result"]}
  ]
}
```
...
{
  "operations": [
    {"method": "apiRequest", "args": ["https://api.stripe.com/v1/checkout/sessions", "POST", {"headers": {"Authorization": "Bearer $env.STRIPE_SECRET_KEY", "Content-Type": "application/x-www-form-urlencoded"}, "params": {"mode": "payment", "success_url": "$success_url", "cancel_url": "$cancel_url"}}, "result"]}
  ]
}
```
...
stream.from_request {
  ...
  method = "POST"
  headers = []|push:"Authorization: Bearer "|add:$env.API_KEY
  body = {query: $input.search_query}
} as stream
