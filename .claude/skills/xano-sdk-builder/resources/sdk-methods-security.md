...
**CRITICAL:** Password hashing methods (`hashPassword()`, `passwordHash()`) are NOT SUPPORTED. Use the `|hash` filter instead:

```json
{
  "operations": [
    {"method": "var", "args": ["hashed_password", "$input.password|hash"]}
  ]
}
```
